    ...
    if state:
        ...
        try:
            row = [
                   ...
                   ctnfe.h_total(Href, Link, Media, Form, CSS, Favicon),
                   ...
                   trdfe.google_index(google_api_key, google_api_cx, url),
...
#################################################################################################################################
#              Generate website content, external and url features
#################################################################################################################################

def generate_external_dataset(url):
    global status
    i = 0
    if not os.path.isfile('data/dataset_checked.csv'):
        with open('data/dataset_checked.csv', 'w') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['url'] + headers + ['status'])
            csvfile.close()

    db = pd.read_csv('data/dataset_checked.csv')
    ...
    if url not in lst:
        ...
        if res != None:
            url_status = model_inference(pd.DataFrame(res, headers))
            res = [url] + res + [url_status[0]]
            with open('data/dataset_checked.csv', 'a') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(res)
                csvfile.close()
            status = url_status[0]
        ...
        res = db[db['url'] == url]
        status = res['status'].iloc[-1]
        state = 'Fn'
    i += 1
    print('[', state, ']', nb, 'succeeded from:', i)
    if state == 'Er':
        return 'Er'
    else:
        return status
